...
# ── Konfiguration ─────────────────────────────────────────────────────────────
COMFYUI_DIR   = r"C:\Users\alfredo\ComfyUI"
FLUX_DIR      = r"C:\Users\alfredo\flux-studio"
COMFYUI_PORT  = 8188
FLUX_PORT     = 3000
...
class FluxLauncher:
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Flux Studio", font=("Segoe UI", 22, "bold"),
                 bg=BG, fg=FG).pack(side="left")
        tk.Label(header, text="Launcher", font=("Segoe UI", 22),
        ...
        self.log = scrolledtext.ScrolledText(
            log_frame, bg=BG_LOG, fg=FG_DIM, font=("Consolas", 9),
    ...
    def _status_row(self, parent, label: str):
        ...
        dot = tk.Label(row, text="●", font=("Segoe UI", 14), bg=BG_CARD, fg=RED)
        dot.pack(side="left")
        tk.Label(row, text=label, font=("Segoe UI", 11, "bold"),
                 bg=BG_CARD, fg=FG, width=14, anchor="w").pack(side="left", padx=8)
        lbl = tk.Label(row, text="Gestoppt", font=("Segoe UI", 10),
    ...
    def _launch_sequence(self):
        ...
        # Flux Studio
        self._log("Starte Flux Studio …", FG)
        self.root.after(0, lambda: self._set_status(
            ...
            if proc and proc.poll() is None:
                try:
                    subprocess.call(
                        ...
                        creationflags=subprocess.CREATE_NO_WINDOW)
                    self._log(f"{name} beendet.", FG_DIM)
                except Exception as e:
                    self._log(f"{name} Fehler beim Beenden: {e}", RED)

        kill(self.proc_flux,    "Flux Studio")
        kill(self.proc_comfyui, "ComfyUI")
        self.proc_flux    = None
